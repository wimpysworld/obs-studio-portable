...
function stage_05_build_obs() {
    ...
    cmake -S "${SOURCE_DIR}/" -B "${BUILD_TO}/" -G Ninja \
      ...
      -DENABLE_VLC=ON \
      -DENABLE_WAYLAND=ON \
      -DRESTREAM_CLIENTID=${RESTREAM_CLIENTID} -DRESTREAM_HASH=${RESTREAM_HASH} \
      -DTWITCH_CLIENTID=${TWITCH_CLIENTID} -DTWITCH_HASH=${TWITCH_HASH} \
      -DYOUTUBE_CLIENTID=${YOUTUBE_CLIENTID} -DYOUTUBE_CLIENTID_HASH=${YOUTUBE_CLIENTID_HASH} -DYOUTUBE_SECRET=${YOUTUBE_SECRET} -DYOUTUBE_SECRET_HASH=${YOUTUBE_SECRET_HASH} \
